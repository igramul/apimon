...
# so only LED_COUNT - 1 LEDs are available for ticket display.
TICKET_LED_COUNT = LED_COUNT - 1
...
def test_set_ticket_exact_fit_order(default_mapper):
    ...
    # (now overflows because LED 0 is reserved for status)
    ...
    expected_leds = [Color.black] + [Color.blue] * 3 + [Color.green] * 3 + [Color.red] * 3
    assert len(default_mapper.leds) == LED_COUNT
...
def test_set_ticket_single_color_with_order(default_mapper):
    ...
    default_mapper.set_ticket(tickets)
    expected_leds = [Color.black] + [Color.green] * 7 + [Color.black] * 2
    assert len(default_mapper.leds) == LED_COUNT
    assert default_mapper.leds == expected_leds
    assert default_mapper.overflow is False
...
def test_set_ticket_large_overflow_with_order(default_mapper):
    ...
    default_mapper.set_ticket(tickets)
    expected_leds = [Color.black] + [Color.blue] * 2 + [Color.green] * 3 + [Color.red] * 4
    assert len(default_mapper.leds) == LED_COUNT
    assert default_mapper.leds == expected_leds
    assert default_mapper.overflow is True
...
def test_set_ticket_partial_distribution_and_order(default_mapper):
    # Test uneven ticket distribution (now overflows because LED 0 is reserved)
    ...
    default_mapper.set_ticket(tickets)
    expected_leds = [Color.black] + [Color.blue] * 2 + [Color.green] * 2 + [Color.red] * 5
    assert len(default_mapper.leds) == LED_COUNT
    assert default_mapper.leds == expected_leds
    assert default_mapper.overflow is True
...
def test_set_ticket_with_min_colors_with_order(default_mapper):
    ...
    default_mapper.set_ticket(tickets)
    expected_leds = [Color.black] + [Color.blue] * 3 + [Color.green] * 1 + [Color.red] * 5
    assert len(default_mapper.leds) == LED_COUNT
    assert default_mapper.leds == expected_leds
    assert default_mapper.overflow is True
